...
def main():
    ...
    if arguments['generate']:
        # quick and dirtyOK cp the links required in .js 
        getInlineAssets = ("mkdir "
                    "-p "
                    "{0}/assets/js "    
                    "&& "
                    "cp "
                    "-rf "
                    "content/themes/Saga/assets/js/helper "
                    "{0}/assets/js").format(static_path); 
        ...
        os.system(getInlineAssets+' && '+getWeb);

        # remove query string since Ghost 0.6.4
        file_regex = re.compile(r'.*?(\?.*)')

        for root, dirs, filenames in os.walk(static_path):
            for filename in filenames:
                if file_regex.match(filename):
                    newname = re.sub(r'\?.*', '', filename)  #fix the regex
                    print newname
